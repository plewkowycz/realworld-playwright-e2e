...
test.describe('Follow Feed', () => {
	test('Article from followed user appears in Your Feed', async ({
		...
		await test.step('User B publishes a new article', async () => {
			...
			await homePage.goto();
			await expect(
				page.getByRole('link', { name: new RegExp(`User profile image\\s+${userB.user.username}`) })
			).toBeVisible();
		...
		await test.step('Article shows up in User A’s Your Feed', async () => {
			...
			await homePage.goto();
			await expect(
				page.getByRole('link', { name: new RegExp(`User profile image\\s+${userA.user.username}`) })
			).toBeVisible();
